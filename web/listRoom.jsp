	...
	<body class="flex flex-col min-h-screen">
		...
		<main class="flex flex-1 h-full bg-cerise-red-500">
			<div   class="flex flex-col items-center justify-start w-full p-4 mx-auto space-y-4 bg-white">
				<form action="/BookingHotel/RoomList" method = "POST" class="w-full space-y-2">
					...
					<a href="/BookingHotel/AddRoom" class="inline-block px-16 py-2 mt-8 font-semibold text-white duration-300 bg-gray-800 rounded-sm hover:bg-gray-600">Add New Room</a>
				</form>
					...
					<tbody>
						...
						<tr class="<%= list.get(i).getState() == 0 ? "bg-red-200" :  list.get(i).getState() == 2 ?" bg-yellow-300": "bg-white" %>"> 
							...
							<td class="p-1 text-center border border-black">
								...
								<a href="/BookingHotel/CheckOut?roomId=<%= list.get(i).getRoomId()%>" onclick="return confirm('Are you sure to checkout') " class="inline-block p-2 font-medium text-white duration-200 bg-blue-500 rounded-sm hover:bg-blue-600">Check Out</a>


								<a href="/BookingHotel/CancelBookingInfo?roomId=<%= list.get(i).getRoomId()%>" onclick="return confirm('Are you sure to cancel') " class="inline-block p-2 font-medium text-white duration-200 bg-red-500 rounded-sm hover:bg-red-600">Cancel</a>


								<% } else { %>

								<a href="/BookingHotel/UpdateRoom?roomId=<%= list.get(i).getRoomId()%>" class="inline-block p-2 font-medium text-white duration-200 bg-blue-500 rounded-sm hover:bg-blue-600">Update</a>
							</td>
							<% } %>
						</tr>
						<% }%>
