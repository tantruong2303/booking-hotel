...
public class AddToCartController extends HttpServlet {
    ...
    private boolean postHandler(HttpServletRequest request, HttpServletResponse response) throws Exception {
        ...
        if (bookingInfoList == null) {
            ...
            // checking intersection date
            BookingInfo bookingInfo = new BookingInfo(updateBookingInfoList.size() + 1, room, startDate, endDate, -1,
                    room.getPrice());
            for (Map.Entry<Integer, BookingInfo> entry : updateBookingInfoList.entrySet()) {
                Integer key = entry.getKey();
                BookingInfo val = entry.getValue();
                if (val.getRoom().getRoomId() == bookingInfo.getRoom().getRoomId()
                        && !Validator.checkDateInRange(val.getStartDate(), val.getEndDate(), startDate, endDate)) {
                    request.setAttribute("errorMessage",
                            "This room have a booking in your cart from "
                                    + Helper.convertDateToString(val.getStartDate()) + " to "
                                    + Helper.convertDateToString(val.getEndDate()) + ", please select other day.");
                    return false;
                }
